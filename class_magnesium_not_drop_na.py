...
class Magnesium(object):
    ...
    def prec_recall(self, y_test, y_prob, plots):
        ...
        if plots:
            ...
            prec_recall_plot.append(self.form_plot_string('plt.plot', recall, precision, color=self.colours[0],
                                                          lw=1,
                                                          label=self.model_name + ' (area = {0:0.2f})'''.format(acc)))
    ...
    def plot_probability_density(self, y_prob=None, y=None, plots=True):
        ...
        if plots:
            probability_density_plot.append(self.form_plot_string('plt.fill_between',
                                                                  x1, kde1(x1), 0, color='darkblue', alpha=0.5,
                                                                  label='Sites'))
            probability_density_plot.append(self.form_plot_string('plt.fill_between',
                                                                  x2, kde2(x2), 0, color='darkgrey', alpha=0.5,
                                                                  label='Non-sites'))
            probability_density_plot.append(self.form_plot_string('plt.axvline',
                                                                  x1[np.argmax(kde1(x1))], color='black',
                                                                  linestyle='--', alpha=0.5))
            probability_density_plot.append(self.form_plot_string('plt.axvline',
                                                                  x2[np.argmax(kde2(x2))], color='black',
                                                                  linestyle='--', alpha=0.5))
            probability_density_plot.append(self.form_plot_string('plt.axvline',
                                                                  treshold, color='black', linestyle='-.', alpha=0.7,
                                                                  label=str(round(treshold, 2))))
    ...
    def plot_confusion_matrix_(self, cm, normalize=False, title='Confusion matrix'):
        ...
        for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
            ...
            general_plot_strings.append(
                self.form_plot_string('plt.text', j, i, round(cm[i, j], 2), horizontalalignment="center", color=colour))
    ...
    def plot_confusion_matrix(self, y_test, y_pred, plots):
        ...
        if plots:
            ...
            cnf_normed_plot = self.plot_confusion_matrix_(cnf_matrix, normalize=True,
    ...
    def form_plot_string(self, type_of_plot, *args, **kwargs):
        ...
        properties = ','.join(
            ['='.join([str(name), str(value.tolist())]) if str(type(value)).split('.')[0] == "<class 'numpy"
             else ('='.join([str(name), '"' + str(value) + '"']) if type(value) == str
                   else '='.join([str(name), str(value)])) for name, value in kwargs.items()])
        return (type_of_plot + '(' + arguments + ',' + properties + ')') if (
                    len(arguments) != 0 and len(properties) != 0) else (
                    type_of_plot + '(' + arguments + properties + ')')
